...
class SwaggerPagingInferrerTest {
    ...
    fun `records field is accepted as list column`() {
        ...
        val p = SwaggerPagingInferrer().enrich(spec).operations.single().paging
        assertNotNull(p)
    ...
    fun `raw list response is not paging`() {
        ...
        assertNull(SwaggerPagingInferrer().enrich(spec).operations.single().paging)
    }
}
