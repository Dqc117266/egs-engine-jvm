...
class GoldenModuleTest {
    ...
    fun `android module golden`() {
        val files = ModuleGenerator().preview(tempRoot(), androidTemplate())
        GoldenSnapshot.verify("module-android", files.map { it.path to it.content })
    }

    @Test
    fun `kmp module golden`() {
        val files = ModuleGenerator().preview(tempRoot(), kmpTemplate())
        GoldenSnapshot.verify("module-kmp", files.map { it.path to it.content })
    }
