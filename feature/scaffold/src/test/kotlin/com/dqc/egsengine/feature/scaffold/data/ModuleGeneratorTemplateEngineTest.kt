...
class ModuleGeneratorTemplateEngineTest {
    ...
    fun `preview uses project local android module template overrides`() {
        ...
        val files = ModuleGenerator().preview(projectRoot, androidTemplate())
    ...
    fun `preview renders kmp module templates into commonMain layout`() {
        ...
        val files = ModuleGenerator().preview(projectRoot, kmpTemplate())

        val viewModel = files.single { it.path.endsWith("ProfileViewModel.kt") }
        assertNotNull(viewModel.content)
        assertTrue(viewModel.content!!.contains("Feature module (KMP)"))
        assertEquals(
            "feature/profile/src/commonMain/kotlin/org/mifos/feature/profile/presentation/profile/ProfileViewModel.kt",
            viewModel.path,
        )
        assertTrue(files.any { it.path.endsWith("ProfileScreen.kt") })
