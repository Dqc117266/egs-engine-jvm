...
class KmpSwaggerTemplateRenderer(
    ...
    fun defaultRepositorySupportStatement(
        op: SwaggerOperation,
        ctx: KmpSwaggerGeneratorContext,
    ): String {
    ...
    fun renderGeneratedDomainModule(
        ...
        val dbUseCases =
            preservedDbUseCaseClassNames.map { simpleName ->
                mapOf("useCaseClass" to simpleName)
            }
        val dbUseCaseImports =
            preservedDbUseCaseClassNames.map { simpleName ->
                "${ctx.domainUseCasePackage}.$simpleName"
            }
        val prefsUseCases =
            preservedPrefsUseCaseClassNames.map { simpleName ->
                mapOf("useCaseClass" to simpleName)
            }
        val prefsUseCaseImports =
            preservedPrefsUseCaseClassNames.map { simpleName ->
                "${ctx.domainUseCasePackage}.$simpleName"
            }
        return engine.render(
